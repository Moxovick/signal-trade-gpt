...
async def cb_step2(query: CallbackQuery) -> None:
    text = (
        ...
        "\n"
        "Доступ к боту и сигналам открывается автоматически, как только PocketOption "
        "подтверждает, что ты зарегался по нашей ссылке.\n"
        "\n"
        "  • <b>T0 · Обычная</b> (бесплатно) — безлимит OTC-сигналов, личный кабинет\n"
        "  • <b>T1 · Pro ≥ $20 депозита</b> — безлимит всё (OTC + биржа + Elite),"
