...
def _format_welcome(first_name: str, ref_code: str, bot_username: str) -> str:
    ...
    return (
        ...
        f"Доступ открывается регистрацией по нашей ссылке, а не подпиской.\n"
        f"\n"
        f"<b>Как начать:</b>\n"
        f"  1. Открой счёт PocketOption по нашей ссылке (/link)\n"
        f"  2. Привяжи Trader ID — сразу получаешь базовый доступ\n"
        f"  3. Внесёшь депозит ≥ $20 — откроется безлимит\n"
        f"\n"
        f"<b>Как работает доступ:</b>\n"
        f"  • <b>T0 · Обычная</b> — безлимит OTC-сигналов сразу после привязки PO\n"
        f"  • <b>T1 · Pro ≥ $20</b> — безлимит всё: OTC + биржа + Elite + индикаторы\n"
        f"\n"
