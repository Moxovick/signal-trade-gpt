...
> ### v2.1 update — 2-tier модель (override)
>
> Текущая модель доступа: **только T0 и T1** (см. `web-platform/src/lib/tier.ts`).
>
> - **T0** — регистрация на PocketOption по нашей реф-ссылке, депозит **не нужен**.
> - **T1 (Pro)** — депозит ≥ **$20** на привязанном счёте.
>
> Регистрация **не** блокируется по сумме депозита; PO Trader ID на `/register` опционален.
> Любые упоминания «$100 / $500 / $1000 / $2000 / $10000» / «T2/T3/T4» в этом документе
> устарели и должны игнорироваться. Многоуровневая модель свёрнута через выставление
> `tier_thresholds.{2,3,4} = Number.MAX_SAFE_INTEGER`; возврат — без миграций кода.
