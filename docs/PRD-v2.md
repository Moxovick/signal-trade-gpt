...
> ### v2.1 update — 2-tier модель (override остального документа)
>
> С 12.05.2026 многоуровневая модель **T0/T1/T2/T3/T4** (с порогами $0/$100/$500/$2000/$10000) **свёрнута в 2 тира**:
>
> - **T0 · Free** — регистрация на PocketOption по нашей реф-ссылке (`click_id={user.id}`).
>   Депозит не требуется. Перки: личный кабинет + 3 OTC-сигнала / день.
> - **T1 · Pro** — общий депозит на привязанном PO-аккаунте ≥ **$20**. Безлимит,
>   все банды (OTC + биржа + Elite), индикаторы (RSI/MACD/EMA/volume), ранний доступ 60с.
>
> Регистрация на сайте **не** блокируется по сумме депозита — PO Trader ID
> опционален, его можно дозаполнить после регистрации на `/onboarding/po-id`.
> Любые упоминания «минимальный депозит $20 → отказ» в этом документе устарели и должны игнорироваться.
>
> Поля `tier_thresholds.{2,3,4}` оставлены в `SiteSettings` выставленными в
> `Number.MAX_SAFE_INTEGER` — возврат к многоуровневой модели делается без
> миграции кода (только правка настроек). См. `web-platform/src/lib/tier.ts`.
